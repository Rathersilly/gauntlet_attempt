...
class Baddie< Ent
  ...
  def initialize traits
    ...
    @anims[:die].remain = true

    @anims[:dead] = Anim.new(name: :dead)#,path: @path)
      @anims[:dead].frames << "sprites/necromancer/adept-dead8.png"
    @anims[:dead].loop = true

    @anims[:transition] = Anim.new(name: :transition)#,path: @path)
    (5..7).each do |i|
